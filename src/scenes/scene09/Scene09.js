...
import { Scene09_ManifoldSphere } from './Scene09_ManifoldSphere.js';
...
export class Scene09 extends SceneBase {
    constructor(renderer, camera, sharedResourceManager = null) {
        ...
        debugLog('scene09', 'コンストラクタ実行', this.title);
        ...
        // Scene09専用の色反転フラグ（ColorInversionの影響を受けずに独自管理）
        this.scene09ColorInverted = false;
        this.scene09ColorInversionEndTime = 0;
        this.scene09ColorInversionKeyPressed = false;
    ...
    async setup() {
        ...
        if (this.useSharedResources && this.sharedResourceManager) {
            // 共有リソースから取得（既に初期化済み）
            this.gpuParticleSystem = this.sharedResourceManager.getGPUParticleSystem('scene09');
            debugLog('scene09', '共有リソースからGPUパーティクルシステムを取得');
            
            // Scene09固有：初期位置データを設定（SharedResourceManagerから分離）
...
this.initializeParticleData();
            ...
            this.gpuParticleSystem = new GPUParticleSystem(
                ...
                this.rows,
                0,  // baseRadiusは使用しない
                'scene09'  // シェーダーパス（public/shaders/scene09）
            );
            
            // シェーダーの読み込み完了を待つ
            await this.gpuParticleSystem.initPromise;
        }
        debugLog('scene09', 'GPUParticleSystem初期化完了');
        ...
        if (particleSystem) {
            ...
            debugLog('scene09', 'パーティクルシステムをシーンに追加', particleSystem);
            ...
            if (this.gpuParticleSystem.particleMaterial) {
                ...
                debugLog('scene09', 'パーティクルマテリアル設定完了');
            }
            ...
            if (sizeAttribute) {
                ...
                debugLog('scene09', 'パーティクルサイズ設定完了', sizes.length);
            }
        } else {
            console.error('Scene09: パーティクルシステムが取得できません');
        }
        ...
        if (this.gpuParticleSystem && this.gpuParticleSystem.particleMaterial && this.gpuParticleSystem.particleMaterial.uniforms) {
            if (this.gpuParticleSystem.positionRenderTargets && this.gpuParticleSystem.positionRenderTargets[0]) {
                if (this.gpuParticleSystem.particleMaterial.uniforms.positionTexture) {
                    ...
                    debugLog('scene09', 'positionTexture設定完了');
                }
            }
            if (this.gpuParticleSystem.colorRenderTargets && this.gpuParticleSystem.colorRenderTargets[0]) {
                if (this.gpuParticleSystem.particleMaterial.uniforms.colorTexture) {
                    ...
                    debugLog('scene09', 'colorTexture設定完了');
                }
            }
        ...
        // 線描画システムを作成（GPUParticleSystem側で作成、チャンク分割によりブロッキングしない）
        ...
        try {
            ...
            console.error('[Scene09] initChromaticAberrationエラー:', err);
        }
    ...
    /**
     * Scene09 は composer 上の colorInversionPass で色反転（ColorInversion クラスは使わない）
     */
    ...
    initTextEffectCanvas() {
        // 既存のCanvasがあれば削除
        const existingCanvas = document.getElementById('scene09-text-effect-canvas');
        if (existingCanvas) {
            existingCanvas.parentElement?.removeChild(existingCanvas);
        }
        
        this.textEffectCanvas = document.createElement('canvas');
        this.textEffectCanvas.id = 'scene09-text-effect-canvas';
    ...
    updateInitialColors() {
        ...
        if (colorUpdateMaterial && this.gpuParticleSystem.colorRenderTargets && this.gpuParticleSystem.colorRenderTargets[0]) {
            ...
            if (this.gpuParticleSystem.particleMaterial && this.gpuParticleSystem.particleMaterial.uniforms) {
                ...
                debugLog('scene09', 'updateInitialColors - テクスチャ設定完了');
            } else {
                console.error('Scene09: updateInitialColors - particleMaterialまたはuniformsが存在しません');
            }
        } else {
            console.error('Scene09: updateInitialColors - colorUpdateMaterialまたはcolorRenderTargetsが存在しません');
        }
    }
    ...
    onUpdate(deltaTime) {
        ...
        // Scene09専用色反転のサスティン終了チェック
        if (this.scene09ColorInverted && !this.scene09ColorInversionKeyPressed) {
            if (this.scene09ColorInversionEndTime > 0 && Date.now() >= this.scene09ColorInversionEndTime) {
                this.scene09ColorInverted = false;
                this.scene09ColorInversionEndTime = 0;
    ...
    render() {
        ...
        // Scene09独自のcomposerでレンダリング（色収差、グリッチなど）
        // 色反転は無効化
        if (this.composer) {
        ...
        // Scene09専用色反転が有効な場合はCanvas 2Dの内容も反転
        if (this.scene09ColorInverted && this.textEffectCtx && this.textEffectCanvas) {
        ...
        if (this.hud) {
            if (this.showHUD) {
                ...
                // Scene09専用の色反転フラグを使用（HUDの色も反転する）
                const isInverted = this.scene09ColorInverted;
    ...
    drawManifoldSphereCallout(sphere) {
        ...
        ctx.save();
        ctx.lineWidth = 2;
        ctx.strokeStyle = this.scene09ColorInverted ? 'rgba(0, 255, 255, 0.78)' : 'rgba(255, 0, 0, 0.78)';
        ...
        ctx.fillStyle = this.scene09ColorInverted ? 'rgba(0, 0, 0, 1.0)' : 'rgba(255, 255, 255, 1.0)';
    ...
    createManifoldSphere(noteNumber = null, velocity = null) {
        ...
        const sphere = new Scene09_ManifoldSphere(position, label);
        sphere.u = u;  // パラメータ空間のu座標を保存
        sphere.v = v;  // パラメータ空間のv座標を保存
        sphere.createThreeObjects(this.scene);
        this.manifoldSpheres.push(sphere);
        
        debugLog('scene09', `マーカースフィアを作成: ${label} at (${position.x.toFixed(1)}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)})`);
        debugLog('scene09', `  パラメータ: u=${u.toFixed(3)}, v=${v.toFixed(3)}`);
        debugLog('scene09', `  現在のマーカースフィア数: ${this.manifoldSpheres.length}`);
    }
    
    
    /**
     * OSCメッセージのハンドリング（Scene09専用オーバーライド）
     * トラック2の色反転はScene09では無効（技術的問題のため）
     */
    handleOSC(message) {
        const trackNumber = message.trackNumber;
        
        // トラック2: Scene09では色反転を無効化（SceneBaseの処理もスキップ）
        if (trackNumber === 2) {
            // 何もせずreturn（色反転を無効）
            return;
        }
    ...
    transformManifold(noteNumber = null, velocity = null) {
        ...
        if (noteNumber === null) {
            ...
            if (mode === 0) {
                ...
                debugLog('scene09', `複雑さを急激に変更: ${this.manifoldComplexity.toFixed(2)}`);
            } else if (mode === 1) {
                // スケールを急激に変更（より極端な値）
                this.manifoldScale = 50.0 + Math.random() * 450.0;  // 50.0 ～ 500.0
                debugLog('scene09', `スケールを急激に変更: ${this.manifoldScale.toFixed(2)}`);
            } else {
                // 両方を急激に変更
                this.manifoldComplexity = 0.3 + Math.random() * 7.7;
                this.manifoldScale = 50.0 + Math.random() * 450.0;
                debugLog('scene09', `複雑さとスケールを急激に変更: ${this.manifoldComplexity.toFixed(2)}, ${this.manifoldScale.toFixed(2)}`);
            }
        } else {
            // ノート番号に基づいて急激に変形
            if (noteNumber < 12) {
                ...
                debugLog('scene09', `複雑さを急激に変更 (Note ${noteNumber}): ${this.manifoldComplexity.toFixed(2)}`);
                ...
                debugLog('scene09', `スケールを急激に変更 (Note ${noteNumber}): ${this.manifoldScale.toFixed(2)}`);
                ...
                debugLog('scene09', `複雑さとスケールを急激に変更 (Note ${noteNumber}): ${this.manifoldComplexity.toFixed(2)}, ${this.manifoldScale.toFixed(2)}`);
            } else {
                // 36以上: リセット
                this.manifoldComplexity = this.baseManifoldComplexity;
                this.manifoldScale = this.baseManifoldScale;
                debugLog('scene09', `リセット: ${this.manifoldComplexity.toFixed(2)}, ${this.manifoldScale.toFixed(2)}`);
            }
        }
    ...
    handleKeyDown(trackNumber) {
        ...
        // トラック2: Scene09専用色反転開始
        if (trackNumber === 2) {
            this.scene09ColorInversionKeyPressed = true;
            this.scene09ColorInverted = true;
    ...
    cleanupSceneSpecificElements() {
        debugLog('scene09', 'cleanupSceneSpecificElements');
        
        // Scene09独自のcolorInversionPassを無効にする（シーン切り替え時にリセット）
        if (this.colorInversionPass) {
    ...
    dispose() {
        ...
        if (this.gpuParticleSystem) {
            ...
            if (this.useSharedResources && this.sharedResourceManager) {
                ...
                this.sharedResourceManager.releaseGPUParticleSystem('scene09');
                debugLog('scene09', '共有リソースを返却');
            } else {
                // 通常の場合はdispose
